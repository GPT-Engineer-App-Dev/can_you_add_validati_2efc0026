...
const Index = () => {
  ...
  const validateLogin = () => {
    ...
    // Check if username is empty or invalid format
    const username = document.getElementById("username").value;
    if (!username || !/^[a-zA-Z0-9_]+$/i.test(username)) {
      setUsernameError("Username is required and must be alphanumeric");
      isValid = false;
    }
    ...
    if (!password || password.length < 8) {
      setPasswordError("Password is required and must be at least 8 characters long");
      isValid = false;
    }
